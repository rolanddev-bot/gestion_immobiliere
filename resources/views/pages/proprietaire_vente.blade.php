...
<a href="{{route('proprietaire_v_create') }}" style="font-size: 15px " class="text-center btn btn-success btn-lg"
...
@foreach($proprietaire as $prop)
      <?php $datenow = \Carbon\Carbon::now()->format('Y'); ?>
        ...
        <td>
        <a href="{{route('proprietaire_v_show',['id'=> $prop->id])}}"  title="Voir détail {{ $prop->type_proprietaire }}">
        @if($prop->nom_societe !='')
        ...
        <td>
            ...
            @endif
        <td>
<a href="{{route('proprietaire_v_edit',['id'=> $prop->id])}}"  title="Modifier" >
