...
<script type="text/javascript">
    ...
    $('body').on('click', '#btnAjouterdes_Element', function() {
        var description = $(this).data('description_id');
        var equiper_id = $(this).data('des_equiper_id');
        console.log(description);


        $('#id_description').val(description);
        $('#des_equiper_id').val(equiper_id);

        var libelle = $(this).data('des_libelle');

        $('#titremodal').html('Elément '+libelle);

        $('#modal_des_Element').modal('show');
    });
...
<p>
   ...
   <a href="{{ url('bien/'.$bien->typebien->id.'/cat')}}" class="btn btn-default"><i class="fas fa-arrow-circle-left"></i> Liste</a>
   |
   <a href="{{ url('bien/'.$bien->id.'/edit')}}" class=""><i class="fas fa-fw fa-edit" style="font-size:15px"></i>Modifier</a>
</p>
...
<div class="row">
  <!--  ***** ajout de Composant du bien   ***********-->
    ...
    <!--  ***** ajout de description du bien   ***********-->
    <div class="col-md-6">

    <form class="" method="POST" action="{{ route('descript_create') }}" style="border:1px solid #333; width:78%; padding:10px; margin:10px">
    @csrf
    ...
    <div class="col-md-8">
        <div class="form-group">
            <select class="form-control" id="description_id" name="description_id" required>
                <option value="">Choisir description</option>

            @foreach($descriptions as $description)
                <option value="{!! $description->id !!}"> {!! $description->libelle !!}</option>
            @endforeach
            </optgroup>
            </select>
            <span class="text-danger">{{ $errors->first('Charge') }}</span>
...
<!--*************************** pour la description du bien **************************************-->
    <div class="col-md-6">
        ...
        <tbody>
        <?php $mt = 0; $mtp = 0; $i = 1; ?>
        @foreach($equipers_des as $eqp)

            <tr style="background-color:#EFEFEF">
            <td>{{ $i++ }}</td>
            <td>{{ $eqp->description->libelle  }}</td>
            <td>{{ $eqp->detail }} </td>
            <td>
        ...
        @if($bien->archiver == 0)
        <a href="javascript:void(0)" data-description_id="{{ $eqp->description->id  }}" data-des_equiper_id="{{ $eqp->id  }}"  data-des_libelle="{{ $eqp->description->libelle  }}"
        data-target="#modal_des_Element" id="btnAjouterdes_Element" > <i class="fas fa-fw fa-plus-square"></i></a>

                <a href="{{ url('des_equipersup/'.$eqp->id) }}" id="" class=""><i class="fas fa-fw fa-trash"></i></a>
            @endif
            </td></tr>


            @foreach($elts_des as $elt_des)

                @if($elt_des->equiper_descript->id == $eqp->id)
                    <tr ><td></td><td colspan="2">

                    {!! $elt_des->libelle !!}

                    @if($elt_des->detail != '') - {{ $elt_des->detail }} @endif

                    </td><td>

        @if($bien->archiver == 0)
                    <a href="{{ url('des_delete/'.$elt_des->id) }}" class="">
                    <i class="fas fa-fw fa-trash"></i>
                    </a>
        @endif
                    </td></tr>
...
<div>
    <div class="modal fade bd-example-modal-xl" id="modal_des_Element" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-ld">
        <div class="modal-content">
...
<form  name="formlocalEL" method="POST" action="{{ url('store_description') }}"  id="formlocalEL" >
    {{ csrf_field() }}

    <input type="hidden" name="id_description" id="id_description" value="">
    <input type="hidden" name="des_equiper_id" id="des_equiper_id" value="">
...
<div class="row">
    ...
    <div class="col-md-12">
        <div class="form-group">
        <strong>Elément <b style="color: red;">*</b></strong>
           <input type="text" class="form-control" name="element_des" id="element_des" value="">
        </div>
    </div>
