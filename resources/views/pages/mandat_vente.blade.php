...
@endif
            ...
            <div class="col-lg-12">
        <h2 class="text-center alert alert-secondary">Mandat Vente</h2>
              <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">

 <a href="{{ route('mandat_vente_create')}}" class="text-center btn btn-success btn-lg" style="font-size:15px" data-target="#"><i class="fas fa-fw fa-plus-square"></i>Nouveau</a>
...
@foreach($mandats as $mandat)
  <tr>
  <td><a href="{{ url('mandat_vente_show/'.$mandat->id)}}"> {{$mandat->ref}} </a></td>
    <td>{{ $mandat->bien->libelle }}</td>
    <td>{{ $mandat->nbre_renouvellement }}</td>
    <td>{{ $mandat->frais_cloture }}</td>
    <td>{{ $mandat->commission }}</td>



    <td style="width:15%">
    <a title="Modifier" href="{{ url('mandat_vente_edit/'.$mandat->id.'/edit') }}" title="Modifié">
